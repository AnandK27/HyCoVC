...
class MVEntropyModel(entropy_models.ContinuousEntropyModel):
...
class MVDensity(nn.Module):
    ...
    def __init__(self, n_channels, init_scale=10., filters=(3, 3, 3), min_likelihood=MIN_LIKELIHOOD, 
        ...
        """
        super(MVDensity, self).__init__(**kwargs)
...
if __name__ == '__main__':
    ...
    use_blocks = True
    vectorize = True
    MV_density = MVDensity(n_channels)
    MV_entropy_model = MVEntropyModel(MV_density)
    MV_entropy_model.build_tables()
    ...
    bits, bpp, bpi = MV_entropy_model._estimate_compression_bits(bottleneck, 
        spatial_shape=toy_shape[2:])

    start_t = time.time()
    encoded, coding_shape, symbols = MV_entropy_model.compress(bottleneck,
    ...
    decoded, decoded_raw = MV_entropy_model.decompress(encoded, n_data,
