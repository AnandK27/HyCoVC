...
import src.helpers.functional as FE
from src.helpers.pwcnet import PWCDCNet
from src.helpers.sampler import shift, warp
...
class PWCNet(PWCDCNet):
    ...
    def __init__(self, path='./pretrained/pwc_net.pth.tar', trainable=False):
        super().__init__()
        if path is not None:
            data = torch.load(path, map_location='cpu')
            if 'state_dict' in data.keys():
